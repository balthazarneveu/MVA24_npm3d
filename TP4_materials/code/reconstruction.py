...
def compute_imls(
    points, normals, scalar_field, grid_resolution, min_grid, size_voxel, knn=30, h=0.01,
    device=DEVICE,
    ...
    for idx in tqdm(range(n_batches)):
        ...
        cloud_id = tree.query(xyz_batch, k=knn, return_distance=False)
        # print("done kdtree")
        ...
        xyz_batch = torch.from_numpy(xyz_batch).to(device).unsqueeze(1)
        ...
        # print(knn_points.shape, xyz_batch.shape, kernel.shape)
        # print(dist.shape)
        sdf = sdf.cpu()
        sdf_list.append(sdf)
